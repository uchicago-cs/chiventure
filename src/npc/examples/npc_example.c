 ...
 * Returns: None.
 */
void end_convo(chiventure_ctx_t *ctx, char *farewell, bool say)
{
    if (say)
    {
        print_to_cli(ctx, farewell);
    }
}
 ...
 */
edge_t *read_input(chiventure_ctx_t *ctx, node_t *n, char *input, char *farewell)
{
    if (strcmp(input, "ignore") == 0)
    {
        end_convo(ctx, farewell, true);
    }

    edge_t *res;

    HASH_FIND(hh, n->edges, input, strlen(input), res);
 ...
 */
node_t *traverse_edge(chiventure_ctx_t *ctx, node_t *n, char *input, 
                      char *farewell)
{
    edge_t *e;
    e = read_input(ctx, n, input, farewell);

    if (e != NULL) 
    {
        print_to_cli(ctx, e->quip);
        n = e->toward;
        print_to_cli(ctx, n->dialogue);
        return n;
    } 
    else 
    {
        print_to_cli(ctx, "What?");
        return NULL;
    }
}
 ...
 * Returns: None.
 */
void run_convo(chiventure_ctx_t *ctx, char *input, convo_t *c)
{
    print_to_cli(ctx, c->nodes->cur_node->dialogue);
    print_to_cli(ctx, " Or, you could #ignore# me.");
    node_t *cur;
    while (c->nodes->cur_node->connection_count != 0) 
    {
        print_to_cli(ctx, "> Talk about: ");
        cur = traverse_edge(ctx, c->nodes->cur_node, input, c->farewell);
        if (cur != NULL) 
        {
            c->nodes->cur_node = cur;
        }
    }
    end_convo(ctx, c->farewell, false);
}
