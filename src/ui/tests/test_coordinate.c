...
void check_add_coord(coord_record_t *coordmap, int x, int y, room_t *r,
						int expected)
{
	int rc;
...
Test(coordinate, added_successfully)
{
    ...
    check_add_coord(coordmap, 3, 4, r, 1)
}
...
void check_assign_dfs(coord_record_t *coordmap, int vertical_hops,
           int horizontal_hops, room_t* room, ctx_t *context, int expected)
{
    int rc;

    rc = assign(coordmap, vertical_hops, horizontal_hops, room, context)

    cr_assert_eq(rc, expected,
                 "The map is %s but "
                 "create_valid_map returned %s",
                 expected? "is not a valid map":"is a valid map",
                 equal? "failure":"success");
}

//Returns SUCCESS if all rooms are assigned coordinates
void check_create_valid_map(coord_record_t *coordmap,
					int expected)
{
