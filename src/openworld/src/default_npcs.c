...
npc_t *get_hostile_npcs()
{
   ...
   // assassin
   item_hash_t *inventory_assassin = NULL;
   HASH_ADD_STR(inventory_assassin, item_id, potion_item);
   item_t *rifle_item = item_new("rifle", "sharp rifle", "Level 1 rifle");
   HASH_ADD_STR(inventory_assassin, item_id, rifle_item);
   item_t *knife_item = item_new("knife", "knife", "sharp silver kitchen knife");
   HASH_ADD_STR(inventory_assassin, item_id, knife_item);
   ...
   npc_t *assassin = calloc(1, sizeof(npc_t));
   assassin->npc_name = "assassin";
   assassin->level = 3;
   assassin->inventory = inventory_assassin;
   assassin->classification = NPC_HOSTILE;
   HASH_ADD_STR(hash, npc_name, assassin);
