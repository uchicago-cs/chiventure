...
Test(skilltrees_main_functions, create_skill_list)
{
    ...
    for (int i = 0; i < 5; i++) {
        cr_assert_null(skill_list->active_skill[i],
                       "Active skill %d is not null", i);
        cr_assert_null(skill_list->passive_skill[i],
...
Test(skilltrees_main_functions, new_skill)
{
    ...
    cr_assert_str_eq(name, skill->name,
                     "new_skill() failed at creating the name.");
    cr_assert_str_eq(description, skill->description,
                     "new_skill() failed at creating the description.");
    cr_assert_eq(cur_level, skill->cur_level,
                 "new_skill() failed at creating the current level.");
    cr_assert_eq(experience, skill->experience,
...
Test(skilltrees_main_functions, free_skill_lists_manually)
{
    ...
    rc = free_passive_skills(skill_list);
    rc2 = free_active_skills(skill_list);
