...
Test(custom_type, obj_t_get_int_lua_args)
{
    ...
    object_t *ot3 = obj_t_init(data, INT_TYPE,
        "../../../tests/custom-scripts/Lua_file/int_test_args.lua");
    ot3 = obj_add_arg(obj_add_arg(ot3, data, INT_TYPE), data3, INT_TYPE);
    data_t got3 = arg_t_get(ot3);
    int rv3 = got3.i;
    cr_assert_eq(rv3, 98, "obj_t_get_int 1: failed int direct retrieval with value %u", rv3);

    object_t *ot4 = obj_t_init(data, INT_TYPE,
        "../../../tests/custom-scripts/Lua_file/int_test_args.lua");
    ot4 = obj_add_arg(obj_add_arg(ot3, data, INT_TYPE), data4, INT_TYPE);
    data_t got4 = arg_t_get(ot4);
    int rv4 = got4.i;
    cr_assert_eq(rv3, 98, "obj_t_get_int 1: failed int direct retrieval with value %u", rv4);
}
...
Test(custom_type, lua_three_args_diff_types)
{
    ...
    data_t got = arg_t_get(ot);
    int rv = got.i;
    cr_assert_eq(rv, -1695127648, "lua_three_args_diff_types: failed integer direct retrieval with %i", rv);
}
...
Test(custom_type, lua_three_args_diff_types_interact)
{
    ...
    data_t got = arg_t_get(ot);
    int rv = got.i;
    cr_assert_eq(rv, 1600456608, "lua_three_args_diff_types: failed integer direct retrieval with %i", rv);
}