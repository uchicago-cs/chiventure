...
Test(AST_block_t, list_remove_AST_block_failure)
{
    ...
    cr_assert_eq(ret_val, FAILURE, "list_remove_AST_block succeeded in deleting second iteman item not in a linked list");
...
Test(AST_block_t, failure_cases)
{
  ...
  ret_val = list_add_AST_block(new_ast, new_ast, 2);
  cr_assert_eq(ret_val, FAILURE, "Added an AST_block that was already in the linked list");
